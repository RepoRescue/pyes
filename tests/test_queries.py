...
class QuerySearchTestCase(ESTestCase):
    def setUp(self):
        ...
        # Create index with both type mappings at once to enable multi-type support
        mappings = {
            "mappings": {
                self.document_type: {'properties': self.get_default_mapping()},
                "test-type2": {"_parent": {"type": self.document_type}}
            }
        }
        self.conn.indices.create_index(self.index_name, mappings)
