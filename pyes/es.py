...
class ES(object):
    ...
    def __perform_request(self, method, url, params=None, body=None, headers=None):
        """
        ...
        :arg headers: dictionary of headers to send with the request
        """
        ...
        for attempt in range(self.connection.transport.max_retries + 1):
            ...
            try:
                status, headers_resp, data = connection.perform_request(method, url, params, body, headers=headers, ignore=ignore, timeout=timeout)
                ...
                return status, headers_resp, data
    ...
    def _send_request(self, method, path, body=None, params=None, headers=None, raw=False, return_response=False):
        ...
        if headers is None:
            headers = {}
        # ES 7.x requires Content-Type header
        ...
        status, headers_resp, data=self.connection.transport.perform_request(method, path, params=params, headers=headers, body=body)
        ...
        if return_response:
            return DotDict({"status":status, "body":data, "headers":headers_resp})
        ...
        # handle the response
        response_body = data
        if six.PY3 and isinstance(response_body, bytes):
